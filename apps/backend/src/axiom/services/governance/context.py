...
async def enrich_context(db: AsyncSession, intent: GovernanceIntent) -> dict[str, Any]:
    project = await db.get(Project, intent.project_id)
    project_settings: dict[str, Any] = dict[str, Any](project.settings) if project is not None else {}
