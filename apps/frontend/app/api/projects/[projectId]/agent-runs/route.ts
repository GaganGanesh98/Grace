...
export async function GET(req: Request, context: Ctx): Promise<NextResponse> {
  ...
  const target = `${getApiUrl()}/v1/agent-runs?project_id=${encodeURIComponent(projectId)}${qs ? `&${qs}` : ""}`;
...
export async function POST(req: Request, context: Ctx): Promise<NextResponse> {
  ...
  const target = `${getApiUrl()}/v1/agent-runs?project_id=${encodeURIComponent(projectId)}`;
