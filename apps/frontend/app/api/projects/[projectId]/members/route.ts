...
export async function GET(req: Request, context: Ctx): Promise<NextResponse> {
  ...
  const target = `${getApiUrl()}/api/v1/projects/${encodeURIComponent(projectId)}/members${qs ? `?${qs}` : ""}`;
...
export async function POST(req: Request, context: Ctx): Promise<NextResponse> {
  ...
  const target = `${getApiUrl()}/api/v1/projects/${encodeURIComponent(projectId)}/members`;
