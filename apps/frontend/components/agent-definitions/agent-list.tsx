...
export function AgentList({
  ...
  if (definitions.length === 0) {
    return (
      <p className="rounded-lg border border-border bg-card p-6 font-mono text-axiom-13 uppercase tracking-wide text-text-tertiary">
        No agents yet.
      </p>
    );
  }
        ...
        >
          <Link
            ...
            className={cn(
              ...
              !d.is_archived && "hover:bg-secondary",
            )}
          >
            <div className="font-mono text-axiom-15 font-medium text-[var(--axiom-electric)] underline-offset-2 hover:underline">
              {d.name}
            </div>
            <p className="mt-1 font-mono text-axiom-12 text-text-secondary">{d.model}</p>
            {d.is_archived ? (
              <p className="mt-1 font-mono text-axiom-11 uppercase text-text-tertiary">Archived</p>
            ) : null}
          </Link>
          {!d.is_archived ? (
            <Button
